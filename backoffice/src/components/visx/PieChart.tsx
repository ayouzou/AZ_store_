                ...
                >
                    {(pie) => (
                        <AnimatedPie<BrowserUsage>
                            {...pie}
                            animate={animate}
                            getKey={(arc) => getLabel(arc.data.label)}
...
export default function PieChartResponsive({
    data,
    keys,
}: any) {
    return (
        <ParentSize className="flex graph-container" debounceTime={10} style={{ width: '100%', height: '40vh' }}>
            {({ width: visWidth, height: visHeight }) => {
                if (visWidth < 10 || visHeight < 10) return null;
                return (
                    <PieChart width={visWidth} height={visHeight} />
                )
            }}
        </ParentSize>
    )
